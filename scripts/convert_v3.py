"""Merge lane v3: original 10 + extra 20 + v3 30 teacher episodes -> LeRobot v3.

FOR A POTENTIAL FUTURE RELAUNCH ONLY - do not start any training from this
script. This dataset exists so a future run can relaunch from 60 episodes
without touching the frozen bundles (out/lerobot, out/lerobot_v2) or any
training run. This script performs CPU-only conversion and validation only.
...
- out/dataset_v3/seed_30..59.npz (v3 30 episodes)
- out/seeds_extra/seed_hashes.json (frozen bundle hashes for seeds 10-29,
  copied verbatim, never regenerated)
- out/seeds_v3/seed_hashes.json (frozen bundle hashes for seeds 30-59,
  copied verbatim, never regenerated)

Output (only writable target besides this script file):
- out/lerobot_v3/ (rebuilt from scratch on every run)
...
- Only episodes with success=True are written; each npz seed file maps to
  exactly one LeRobot episode, in seed order (0..59).
...
- ONLY differences: input set is 10 original + 20 extra + 30 v3 npz files
  merged in seed order; output root is out/lerobot_v3 instead of
  out/lerobot; dataset stats are recomputed over all 60 episodes by
  finalize(); and the frozen seed_hashes.json files for seeds 10-29 and
  30-59 are copied verbatim to out/lerobot_v3/seed_hashes_extra.json and
  out/lerobot_v3/seed_hashes_v3.json for provenance (read-only copies,
...
V3_DATASET = ROOT / "out" / "dataset_v3"
LEROBOT_V1_ROOT = ROOT / "out" / "lerobot"
LEROBOT_V3_ROOT = ROOT / "out" / "lerobot_v3"
HASHES_EXTRA_SRC = ROOT / "out" / "seeds_extra" / "seed_hashes.json"
HASHES_V3_SRC = ROOT / "out" / "seeds_v3" / "seed_hashes.json"
HASHES_EXTRA_DST = LEROBOT_V3_ROOT / "seed_hashes_extra.json"
HASHES_V3_DST = LEROBOT_V3_ROOT / "seed_hashes_v3.json"
...
def _ordered(seed_dir: Path, lo: int, hi: int) -> list:
    files = sorted(seed_dir.glob("seed_*.npz"),
                   key=lambda p: int(p.stem.split("_")[1]))
    files = [p for p in files if lo <= int(p.stem.split("_")[1]) <= hi]
    assert [int(p.stem.split("_")[1]) for p in files] == list(range(lo, hi + 1)), (
        f"expected seeds {lo}..{hi} in {seed_dir}")
    return files


def main() -> int:
    orig_files = _ordered(ORIG_DATASET, 0, 9)
    extra_files = _ordered(EXTRA_DATASET, 10, 29)
    v3_files = _ordered(V3_DATASET, 30, 59)
    files = orig_files + extra_files + v3_files
    seeds = [int(p.stem.split("_")[1]) for p in files]
    assert seeds == list(range(60)), "expected seeds 0..59 in order"
    if LEROBOT_V3_ROOT.exists():
        shutil.rmtree(LEROBOT_V3_ROOT)
    dataset = LeRobotDataset.create(
        repo_id=REPO_ID,
        root=LEROBOT_V3_ROOT,
        fps=FPS,
        features=FEATURES,
        use_videos=False,
    )
    ...
    assert written == 60, f"expected 60 episodes, wrote {written}"
    dataset.finalize()
    # Provenance: verbatim read-only copies of frozen hashes (never regenerate).
    shutil.copyfile(HASHES_EXTRA_SRC, HASHES_EXTRA_DST)
    shutil.copyfile(HASHES_V3_SRC, HASHES_V3_DST)
    assert _sha256(HASHES_EXTRA_SRC) == _sha256(HASHES_EXTRA_DST), "extra hash copy mismatch"
    assert _sha256(HASHES_V3_SRC) == _sha256(HASHES_V3_DST), "v3 hash copy mismatch"
    extra_hashes = json.loads(HASHES_EXTRA_DST.read_text())
    v3_hashes = json.loads(HASHES_V3_DST.read_text())
    assert sorted(int(k) for k in extra_hashes) == list(range(10, 30)), "extra hash keys must be 10..29"
    assert sorted(int(k) for k in v3_hashes) == list(range(30, 60)), "v3 hash keys must be 30..59"
    print(f"hashes copied verbatim: {HASHES_EXTRA_SRC} -> {HASHES_EXTRA_DST} "
          f"sha256={_sha256(HASHES_EXTRA_DST)[:16]}...")
    print(f"hashes copied verbatim: {HASHES_V3_SRC} -> {HASHES_V3_DST} "
          f"sha256={_sha256(HASHES_V3_DST)[:16]}...")
    # Validation: API reload.
    reloaded = LeRobotDataset(repo_id=REPO_ID, root=LEROBOT_V3_ROOT)
    ...
    assert reloaded.num_episodes == 60, reloaded.num_episodes
    ...
    v3_info = json.loads((LEROBOT_V3_ROOT / "meta" / "info.json").read_text())
    auto = {"timestamp", "frame_index", "episode_index", "index", "task_index"}
    v1_user = {k: v for k, v in v1_info["features"].items() if k not in auto}
    v3_user = {k: v for k, v in v3_info["features"].items() if k not in auto}
    assert v1_user == v3_user, "schema delta vs out/lerobot (must be none)"
    assert v3_info["fps"] == v1_info["fps"] == FPS
    assert v3_info["total_episodes"] == 60, v3_info["total_episodes"]
    assert v3_info["total_frames"] == expected_frames, v3_info["total_frames"]
    print(f"schema match vs out/lerobot: OK ({len(v3_user)} user features, "
          f"delta=none, fps={FPS})")
    # Validation: one npz -> parquet row-count cross-check (seed_30).
    import pandas as pd

    with np.load(V3_DATASET / "seed_30.npz", allow_pickle=True) as z:
        npz_rows = int(np.asarray(z["frame_state"]).shape[0])
    parquet_files = sorted((LEROBOT_V3_ROOT / "data").rglob("*.parquet"))
    assert parquet_files, "no parquet files in out/lerobot_v3/data"
    ...
    print(f"cross-check: npz total={expected_frames} parquet total={parquet_rows} "
          f"reloaded frames={reloaded.num_frames} (seed_30 npz rows={npz_rows})")
    print(f"lerobot_v3 dataset OK: {written} episodes, {expected_frames} frames "
          f"at {LEROBOT_V3_ROOT}")
